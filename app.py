...
def show_enrolled_courses():
    ...
    cursor.execute("""
        ...
        FROM student_courses sc
        JOIN courses c ON sc.course_id = c.course_id
        JOIN users u ON sc.faculty_id = u.user_id
        WHERE sc.srn = %s
        ...
        if selected_course:
            ...
            if selected_assignment:
                ...
                cursor.execute("""
                    ...
                    JOIN student_courses sc ON ass.student_id = sc.srn AND sc.course_id = %s
                    WHERE ass.assignment_id = %s AND sc.faculty_id = %s
                    ORDER BY ass.submission_date DESC
                """, (selected_course['course_id'], selected_assignment['assignment_id'], st.session_state.user['user_id']))
    ...
    try:
        ...
        if selected_course:
            ...
            cursor.execute("""
                ...
                JOIN student_courses sc ON u.user_id = sc.srn AND sc.course_id = a.course_id
                WHERE a.course_id = %s AND sc.faculty_id = %s
                ORDER BY a.deadline DESC, u.name ASC
            """, (selected_course['course_id'], st.session_state.user['user_id']))
            submissions = cursor.fetchall()
    ...
    if title and description and deadline and st.button("Upload", key="upload_assignment_button"):
        ...
        try:
            ...
            cursor.execute("""
                ...
                SELECT sc.srn, %s
                FROM student_courses sc
                WHERE sc.course_id = %s AND sc.faculty_id = %s
            """, (f"New assignment '{title}' has been uploaded for {course['course_name']}. Deadline: {deadline}", course['course_id'], st.session_state.user['user_id']))
                    ...
                    if assignments:
                        ...
                        for assignment in assignments:
                            ...
                            if submission:
                                ...
                                feedback = st.text_area(
                                    f"Feedback for {assignment['title']}", 
